...
<div id="drawer-navigation" class="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white w-80 dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-navigation-label">
 ...
 <span class="flex-1 ml-3 whitespace-nowrap">Login</span>
  ...
  <path fill-rule="evenodd" d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM3.751 20.105a8.25 8.25 0 0116.498 0 .75.75 0 01-.437.695A18.683 18.683 0 0112 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 01-.437-.695z" clip-rule="evenodd" />
            ...
            <div class="container mx-auto flex justify-between items-center px-6">
                <div>
                    <a href="{{ url('/') }}" class="text-lg font-semibold text-gray-100 no-underline">
                        {{ config('app.name', 'NewsNow') }}
                    </a>
                </div>
                <form class="flex-row" action="{{ route('search') }}" method="GET">
                        <div class="flex" style="min-width:30rem;">
    ...
    @foreach ($tags as $tag )
        <li>
            <button type="submit" class="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" formaction="{{ route('search.tag', $tag->name) }}">{{ $tag->name }}</button>
        </li>    
    @endforeach
@endif
                            </ul>
                ...
                <nav class="space-x-4 text-gray-300 text-sm sm:text-base">
                    ...
                    @guest
                        ...
                        @endif
                    @else
                    <a class="no-underline hover:underline" href="{{ route('user.profile') }}">{{ Auth::user()->name }}</a>
                        <a href="{{ route('logout') }}"
